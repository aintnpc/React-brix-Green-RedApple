...
const PALETTE: { type: StickerType; emoji: string; label: string }[] = [
  { type: 'distance', emoji: '📍', label: '거리' },
  { type: 'time',     emoji: '⏱',  label: '시간' },
  { type: 'kcal',     emoji: '⚡',  label: '칼로리' },
  { type: 'streak',   emoji: '🔥',  label: '연속' },
  { type: 'goal',     emoji: '📊',  label: '달성률' },
  { type: 'day',      emoji: '🗓',  label: '챌린지' },
  { type: 'route',    emoji: '🗺',  label: 'GPS' },
]
    ...
    <View style={[rw.card, selected && rw.cardSel]}>
      <View style={rw.mapArea}>
        {data.route && data.route.length > 1
          ...
          : <View style={rw.noGps}><Text style={rw.noGpsTxt}>GPS 없음</Text></View>
        }
      </View>
...
function getStickerContent(type: StickerType, data: ShareData) {
  switch (type) {
    ...
    case 'streak':   return { label: 'STREAK',    value: String(data.streak),               unit: '일 연속 🔥' }
...
function StickerCanvas({
  data, photoUri, stickers, selectedId, onSelect, onRemove, onScaleChange,
}: {
  data: ShareData
  photoUri?: string
...
export function ShareStickerModal({ visible, data, onClose }: {
  ...
  const takePhoto = async () => {
    ...
    if (perm.status !== 'granted') { Alert.alert('카메라 권한이 필요해요'); return }
      ...
      <View style={[m.root, { paddingTop: insets.top }]}>
        ...
        <View style={[m.sheet, { paddingBottom: Math.max(insets.bottom, 16) }]}>
          ...
          <View style={m.toolRow}>
            <ToolBtn emoji="🖼" label="사진 선택" onPress={pickPhoto} />
            <ToolBtn emoji="📷" label="사진 찍기" onPress={takePhoto} />
            {stickers.length > 0 && (
              <ToolBtn emoji="🗑" label="전체 삭제" onPress={() => { setStickers([]); setSelectedId(null) }} />
            )}
          </View>
