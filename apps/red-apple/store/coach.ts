...
interface CoachState {
  ...
  recalculate: (bodyInfo: BodyInfo, programStartedAt: string, todayConsumed: number, todayBurned: number) => void
}
...
export const useCoachStore = create<CoachState>()(
  persist(
    (set, get) => ({
      ...
      recalculate: (bodyInfo, programStartedAt, todayConsumed, todayBurned) => {
        const { pastMealKcalByDate, pastExerciseKcalByDate } = get()
        const goals = calculateTodayGoals(bodyInfo, programStartedAt, pastMealKcalByDate, pastExerciseKcalByDate, todayConsumed, todayBurned)
        console.log('[Coach] recalculate — consumed:', todayConsumed, 'burned:', todayBurned, 'calorieGoal:', goals?.calorieGoal, 'exerciseGoal:', goals?.exerciseGoalKcalAdjusted ?? goals?.exerciseGoalKcal)
        set({ goals })
      },
    }),
    {
