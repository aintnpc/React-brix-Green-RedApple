...
interface WeightLogState {
  ...
  syncFromDB: (userId: string, programStartedAt?: string | null) => Promise<void>
}
...
export const useWeightLogStore = create<WeightLogState>()(
  persist(
    (set, get) => ({
      ...
      syncFromDB: async (userId, programStartedAt) => {
        ...
        set((state) => ({
          ...
          startDate: entries.length > 0
            ? entries[0].date
            : (programStartedAt ? programStartedAt.slice(0, 10) : state.startDate),
        }))
      },
    }),
    {
